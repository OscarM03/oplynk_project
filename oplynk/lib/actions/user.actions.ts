...
import { uploadFile } from "./session.actions";
...
export const updateUser = async ({
	...
	if (image) {
	  profileImageId = await uploadFile({ image });
	}
...
export const updateUserBalance = async (amount: number, userId: string) => {
    const { databases } = await createAdminClient();

    try {
        // Fetch the latest user document to ensure data consistency
        ...
        // Ensure the balance is defined and update it
        ...
        console.error("Error updating user balance", { userId: userId, error });
